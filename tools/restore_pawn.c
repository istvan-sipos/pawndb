 ...
 *
 * Path B of the design in HANDOFF_RESTORE_PAWN.md: at writeback / archive
 * time the mod stashes each pawn's full <class type="cSAVE_DATA_CMC">…
 * </class> XML region as a sidecar (<HEX>.xml) next to the .pawn archive.
 * The sidecar is well-formed XML — a single root element — so any XML tool
 * can parse it. This tool reads that sidecar and overwrites the target
 * save's own mCmc[0] (main-pawn) region with the same bytes, then
 * re-deflates and repacks. Result: in-game, the pawn that lived inside
 * that archive becomes the player's main pawn — vocation, skills,
 * augments, inclinations, gear, study flags all preserved. (Visible
 * appearance is read by the game from a separate source and is not
 * replaced — see HANDOFF_RESTORE_PAWN.md "Appearance is *not* restored".)
 *
 * Usage:
 ...
 *
 * <DDDA.sav>:
 *   Target save file. Modified in place. A copy is written to <path>.bak
 *   first unless --no-backup is passed.
 *
 *   --no-backup   Skip the .bak. Default is to keep one — recommended.
 *   --force       Reserved; no effect in v1 (no interactive prompt yet).
 *
 * Exit codes:
 *   0  success
 *   1  argument / IO error
...
/* See pawndb.c — same convention. The Makefile injects PAWNDB_VERSION at
 * compile time from the top-level VERSION file. */
#ifndef PAWNDB_VERSION
#define PAWNDB_VERSION "dev"
#endif

extern int ezcompress  (unsigned char *pDest, long *pnDestLen,
                        const unsigned char *pSrc, long nSrcLen);
extern int ezuncompress(unsigned char *pDest, long *pnDestLen,
                        const unsigned char *pSrc, long nSrcLen);
...
static void print_usage(FILE *f)
{
    fprintf(f,
        ...
        "  <DDDA.sav>            Target save. Overwritten in place; .bak written\n"
        "                        first unless --no-backup.\n"
        "\n"
        "  --no-backup           Do not write <DDDA.sav>.bak.\n"
        "  --force               Reserved; no effect in v1.\n"
...
int main(int argc, char *argv[])
{
    ...
    long inflated_len = (long)real_size;
    int rc = ezuncompress(xml, &inflated_len,
                          save_bytes + SAVE_HEADER_SZ, (long)comp_size);
    if (rc != 0 || (uint32_t)inflated_len != real_size) {
        fprintf(stderr,
                "restore_pawn: inflate failed (rc=%d, %ld of %u bytes)\n",
    ...
    /* 7. Re-deflate. EZ_COMPRESSMAXDESTLENGTH(n) ≈ 1.001 * n + 12. */
    long comp_cap = (long)new_xml_len + (long)(new_xml_len / 1000) + 64;
    uint8_t *new_comp = (uint8_t *)malloc((size_t)comp_cap);
    if (!new_comp) {
        fprintf(stderr, "restore_pawn: out of memory (%ld bytes)\n", comp_cap);
        free(new_xml); free(save_bytes); free(sidecar);
        return 1;
    }
    long new_comp_len = comp_cap;
    rc = ezcompress(new_comp, &new_comp_len, new_xml, (long)new_xml_len);
    if (rc != 0) {
        fprintf(stderr, "restore_pawn: deflate failed (rc=%d)\n", rc);
        free(new_comp); free(new_xml); free(save_bytes); free(sidecar);
        return 1;
    }
    printf("deflated:   %ld bytes\n", new_comp_len);
    free(new_xml);

    if ((uint32_t)new_comp_len > SAVE_FIXED_SIZE - SAVE_HEADER_SZ) {
        fprintf(stderr,
                "restore_pawn: re-deflated payload (%ld) exceeds save body capacity (%u). "
    ...
    /* 9. Backup, then overwrite. */
    if (do_backup) {
        char bak_path[1024];
        int n = snprintf(bak_path, sizeof(bak_path), "%s.bak", sav_arg);
        if (n <= 0 || (size_t)n >= sizeof(bak_path)) {
            fprintf(stderr, "restore_pawn: save path too long for .bak suffix\n");
            free(new_save); free(save_bytes); free(sidecar);
            return 1;
        }
        if (write_file_all(bak_path, save_bytes, save_len) != 0) {
            fprintf(stderr, "restore_pawn: cannot write backup '%s'\n", bak_path);
            free(new_save); free(save_bytes); free(sidecar);
            return 1;
        }
        printf("backup:     %s (%lu bytes)\n",
               bak_path, (unsigned long)save_len);
    } else {
    ...
    if (write_file_all(sav_arg, new_save, SAVE_FIXED_SIZE) != 0) {
        fprintf(stderr, "restore_pawn: cannot write '%s' (target save left in indeterminate state — restore from .bak)\n",
                sav_arg);
